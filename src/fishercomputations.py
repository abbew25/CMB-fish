...
def compute_cov(cosmoClval: npt.NDArray, lval: float, noise_Planck: bool = True):
    ...
    deltabB = deltabE
    deltaT = np.array([2.5, 2.2, 4.8e-6]) * deltabT * 1.0e6 * T_cmb
    deltaE = np.array([6.7, 4.0, 9.8e-6]) * deltabE * 1.0e6 * T_cmb
    deltaB = np.array([6.7, 4.0, 9.8e-6]) * deltabB * 1.0e6 * T_cmb
